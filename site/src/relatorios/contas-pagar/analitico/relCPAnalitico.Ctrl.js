'use strict'
angular.module('RelatoriosModule').controller('RelatorioContasPagarAnaliticoCtrl',
	function ($scope, config, $sce, $http, $window, UtilsService, BioacessoService, TitulosProvisionadosService, HeaderFactory) {

        HeaderFactory.setHeader('Relatórios', 'Despesas / Analítico');
		$scope.data = {
			data_inicial: '',
			data_final: '',
			data_ini_pgto: '',
		...
		$scope.goToconsultaGeralPdf = function () {
			if ((this.data.data_inicial != '' && this.data.data_final != '') || (this.data.data_ini_pgto != '' && this.data.data_fim_pgto != '')) {
				...
				var url = config.apiUrl + 'api/relatorios/despesas/contas_a_pagar';
